...
(function(){
  function App() {
    ...
    // ── ANNO SCOLASTICO ──
    var annoDefault = (function(){ try { return SB.LS.anno.get(); } catch(e) { return "2026/2027"; } })();
    ...
    // Deep linking: apri la card indicata da ?card= all'avvio
    useEffect(function(){
      if(deepLinkDone.current) return;
      var id = new URLSearchParams(location.search).get('card');
      if(!id || !cardsHook.cards.length) return;
      var c = cardsHook.cards.find(function(x){ return String(x.id) === String(id); });
      if(c){ deepLinkDone.current = true; setShowCard(c); markSeen(c.id); }
    }, [cardsHook.cards]);
    ...
    // myName() restituisce un nome sicuro per Firestore docIds.
    // SB.safeDocId sostituisce i caratteri proibiti (/, ., [, ], #, $, *) con _.
    // Per studenti italiani tipici non ha alcun effetto; protegge solo da edge case rari.
    function myName(u){ return u ? (u.role === "prof" ? "Prof" : SB.safeDocId(u.displayName || (u.nome + " " + u.cognome).trim() || u.email)) : "?"; }
    function closeCard(){ setShowCard(null); setQRisposte({}); setQInviato(false); setQLoading(false); setEditingCm(null); setReplyTo(null); try { var url = new URL(location.href); url.searchParams.delete('card'); history.replaceState(null, '', url.toString()); } catch(e){} }
    var openCard = useCallback(function(c){ setShowCard(c); markSeen(c.id); try { history.replaceState(null, '', '?card=' + encodeURIComponent(c.id)); } catch(e){} }, []);
    ...
    function addCard(){
      ...
      if(editMode){
        var c = Object.assign({}, editMode, { tipo: form.tipo, titolo: form.titolo.trim(), testo: form.testo.trim(), links: cleanLinks, classi: form.classi, immagini: cleanImmagini, copertina: form.copertina || null, allegati: form.allegati || [] });
        ...
        var c = { id: Date.now(), tipo: form.tipo, titolo: form.titolo.trim(), testo: form.testo.trim(), data: new Date().toISOString().slice(0, 10), autore: myName(user), likes: 0, commenti: [], ordine: cardsHook.nextOrd.current++, links: cleanLinks, visibile: true, classi: form.classi, immagini: cleanImmagini, copertina: form.copertina || null, allegati: form.allegati || [], annoScolastico: annoScolastico };
        if(opzioni) c.opzioni = opzioni;
        if(quizDomande) c.quizDomande = quizDomande;
        if(form.tipo === "quiz") c.quizTimer = form.quizTimer || 10;
    ...
    function editCard(card){
      ...
      setForm({ tipo: card.tipo, titolo: card.titolo, testo: card.testo || "", opzioni: card.opzioni ? card.opzioni.map(function(o){ return o.testo; }) : ["", ""], links: links.length ? links : [{ url: "", label: "" }], classi: card.classi || ["TUTTE"], quizDomande: card.quizDomande || [], quizTimer: card.quizTimer || 10, immagini: card.immagini || [], allegati: card.allegati || [], copertina: card.copertina || null });
    ...
    async function handleImgUpload(e, isCover){
      ...
      // Allowlist MIME esplicita e stretta: blocca SVG e image/sconosciute all'ingresso,
      // prima di arrivare in compressImage (che comunque produce JPEG alla fine).
      var imgMimeOk = function(t){ return t === "image/jpeg" || t === "image/png" || t === "image/gif" || t === "image/webp"; };
      try {
        for(var fi=0; fi<files.length; fi++){
          var file = files[fi];
          if(!imgMimeOk(file.type)){ showToast("Tipo immagine non supportato: " + file.name + " (" + file.type + ")", "warn"); continue; }
    ...
    function delCard(id){ modals.setConfirmDel({ type: "card", id: id }); }
    ...
    async function inviaRisposteQuiz(card){
      ...
      try {
        ...
        console.error("Errore invio quiz:", e);
    ...
    var APP_LAYOUT_FIELDS = {
      ...
      badgeBg: badgeBg,
      tipoIcon: tipoIcon,
      renderLinks: function(card) { return renderLinks(card, setShowCard); },
