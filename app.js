(function(){function App(){
  ...
  var CLASSI_LIST=useMemo(function(){
    return CLASSI_DEFAULT.concat(classiCustom.filter(function(c){return CLASSI_DEFAULT.indexOf(c)<0;}));
  },[classiCustom]);
  var aiCardClasses=useMemo(function(){
    return cards.reduce(function(acc,c){(c.classi||[]).forEach(function(cl){if(acc.indexOf(cl)<0)acc.push(cl);});return acc;},[]).sort();
  },[cards]);
  ...
  useEffect(function(){
    function onKey(e){
      ...
      if(showModal){setShowModal(false);setForm(Object.assign({},FORM0));setEditMode(null);return;}
  ...
  useEffect(function(){
    ...
    return function(){u1();safeUnsub(u2)();u3();u4();safeUnsub(u5)();u6();};
  },[user]);
  ...
  var totC=useMemo(function(){return cards.reduce(function(a,c){return a+(c.commenti||[]).length;},0);},[cards]);
  var proposte=useMemo(function(){return cards.filter(function(c){return c.proposta===true;});},[cards]);
  var isProf=user&&user.role==="prof";
  var simulaSt=isProf&&previewSt;

  var visible=useMemo(function(){return cards.filter(function(c){
    ...
    return true;
  });},[cards,undoDelete,simulaSt,isProf,previewClasse,user,filterClasse]);

  var visibleSorted=useMemo(function(){return visible.slice().sort(function(a,b){
  ...
  function addCard(){
    ...
    setShowModal(false);setForm(Object.assign({},FORM0));
  }
  ...
  function runCardAI(card,e){
    ...
    callGroqJSON(null,prompt,700)
      ...
      .then(function(){setCardAiLoad(null);});
  }
  ...
  return h("div",{style:{minHeight:"100vh",background:"linear-gradient(160deg,#12111a 0%,#161320 50%,#1a1528 100%)",display:"flex",flexDirection:"column"}},h("div",{style:{height:3,background:"linear-gradient(90deg,#6366f1,#8b5cf6,#a855f7,#ec4899)",flexShrink:0}}),
      ...
      h("div",{style:{flex:1,padding:"10px 14px 18px"}},
        cards.length===0
          ...
          ?h("div",{style:{display:"flex",flexDirection:"column",alignItems:"center",padding:"60px 20px",color:"rgba(255,255,255,.2)"}},
              ...
              isProf&&h("button",{onClick:function(){setEditMode(null);setForm(Object.assign({},FORM0));setShowModal(true);},style:{background:"linear-gradient(135deg,#6366f1,#8b5cf6)",border:"none",color:"#fff",borderRadius:8,padding:"7px 18px",fontSize:12,fontWeight:700,cursor:"pointer"}},"+ Aggiungi la prima card")
            )
          :h("div",{className:"card-grid",style:{columns:"300px",columnGap:16}},
            visibleSorted.map(function(c){
              ...
              return h("div",{
                ...
                h("div",{style:{cursor:"pointer",position:"relative"},onClick:function(){openCard(c);}},
      ...
      h("div",{style:{background:"rgba(255,255,255,.05)",borderRadius:11,padding:16,border:"1px solid rgba(255,255,255,.08)"}},
        h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}},
          ...
          h("div",{style:{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}},
            h("select",{value:aiTarget,onChange:function(e){setAiTarget(e.target.value);setAiResult(null);setAiErr("");},style:{padding:"5px 10px",border:"1px solid rgba(255,255,255,.15)",borderRadius:8,fontSize:12,color:"#f1f5f9",background:"rgba(30,41,59,1)",maxWidth:260}},
    ...
    // QR
    showQR&&h("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,.65)",backdropFilter:"blur(3px)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:20},onClick:function(){setShowQR(false);}},
      h("div",{style:{background:"rgba(15,23,42,.92)",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",border:"1px solid rgba(255,255,255,.11)",borderRadius:20,boxShadow:"0 24px 60px rgba(0,0,0,.5)",padding:26,maxWidth:300,width:"100%",textAlign:"center"},onClick:function(e){e.stopPropagation();}},
        ...
        h("button",{onClick:function(){setShowQR(false);},style:{background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"#fff",border:"none",borderRadius:10,padding:"9px 24px",cursor:"pointer",fontSize:13,fontWeight:700,width:"100%"}},"Chiudi")
      )
    ),
      ...
      h("div",{className:"modal-inner",style:{background:"rgba(15,23,42,.96)",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",border:"1px solid rgba(255,255,255,.11)",borderRadius:"20px 20px 0 0",boxShadow:"0 -8px 60px rgba(0,0,0,.6)",width:"100%",maxWidth:560,maxHeight:"92vh",overflow:"auto"},onClick:function(e){e.stopPropagation();}},
        ...
        h("div",{style:{background:showCard.copertina?"transparent":"linear-gradient(135deg,rgba(99,102,241,.3),rgba(139,92,246,.2))",borderRadius:showCard.copertina?"0":"20px 20px 0 0",padding:18}},
          ...
          !isProf&&showCard.autore===myName(user)&&!showCard.proposta&&h("div",{style:{display:"flex",gap:6,flexWrap:"wrap",marginTop:10}},
            h("button",{onClick:function(){editCard(showCard);setShowCard(null);setQRisposte({});setQInviato(false);setQLoading(false);},style:{background:"rgba(59,130,246,.2)",color:"#60a5fa",border:"1px solid rgba(59,130,246,.3)",borderRadius:7,padding:"3px 10px",cursor:"pointer",fontSize:11,fontWeight:700}},"✏️ Modifica")
          ),
    ...
    // MODAL NUOVA CARD
    showModal&&h("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",backdropFilter:"blur(6px)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"},onClick:function(){setShowModal(false);setEditMode(null);}},
