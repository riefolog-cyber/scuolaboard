(function(){function App(){
  ...
  var aiCardClasses=cards.reduce(function(acc,c){(c.classi||[]).forEach(function(cl){if(acc.indexOf(cl)<0)acc.push(cl);});return acc;},[]).sort();
  ...
  function cardPreviewForAI(card){
    ...
    var commCount=(card.commenti||[]).length;
    var likes=card.likes||0;
    var opzioni=card.opzioni?card.opzioni.length:0;
    var quizCount=card.quizDomande?card.quizDomande.length:0;
    var text=(card.testo||"").replace(/\s+/g," ").trim().slice(0,160);
    return "Classi: "+classi+"\n"+
      "Commenti: "+commCount+"\n"+
      "Likes: "+likes+"\n"+
      "Quiz domande: "+quizCount+"\n"+
      "Sondaggi: "+opzioni+"\n"+
      "Testo: "+(text||"(nessun testo)")+"\n";
  }
  ...
  function runAI(){
    ...
    if(aiTarget==="tutte"){
      target=cards;
    }else if(aiTarget.indexOf("classe_")===0){
      var classeName=aiTarget.slice(7);
      target=cards.filter(function(c){return (c.classi||[]).indexOf(classeName)>=0;});
    }else{
      target=[];
    }
    if(!target.length){setAiErr("Nessuna card.");setAiRunning(false);return;}
    var cardSummary=target.map(function(c){return "- \""+c.titolo+"\" (tipo:"+c.tipo+", commenti:"+((c.commenti||[]).length)+", likes:"+(c.likes||0)+")";}).join("\n");
    var det=target.map(function(c){return "CARD: \""+c.titolo+"\" (tipo:"+c.tipo+")\n"+cardPreviewForAI(c);}).join("\n\n---\n\n");
    var prompt='Sei un docente esperto in una scuola secondaria di II grado. Analizza i dati della bacheca digitale con un focus didattico preciso sulla classe selezionata. Usa solo i contenuti reali: temi, commenti, partecipazione e tendenze emergenti.';
    if(aiTarget==="tutte"){
      prompt+='\nLa selezione include tutte le classi. Identifica eventuali differenze di coinvolgimento e temi tra le classi, ma mantieni il resoconto concentrato sulla qualità pedagogica complessiva del gruppo.';
      prompt+='\nCita almeno una misura di partecipazione e un tema ricorrente, e suggerisci un punto di attenzione didattico generale.';
    } else if(aiTarget.indexOf("classe_")===0){
      var classeName=aiTarget.slice(7);
      prompt+='\nLa classe selezionata è '+classeName+' e ci sono '+target.length+' card da considerare. Devi analizzare TUTTE le card di questa classe, comprese quelle senza commenti, e descrivere l’attività didattica complessiva.';
      prompt+='\nNon limitarti ai commenti esistenti: usa anche titoli, testi, tipi di contenuto, quiz e sondaggi per tracciare una panoramica generale.';
    ...
    view==="analisi"&&isProf&&!simulaSt&&h("div",{style:{flex:1,padding:"18px 14px",maxWidth:820,margin:"0 auto",width:"100%"}},
      h("div",{style:{display:"flex",gap:10,flexWrap:"wrap",marginBottom:16}},
        [{l:"Card",v:cards.length,i:"📌",c:"#6366f1"},{l:"Domande",v:cards.filter(function(c){return c.tipo==="domanda";}).length,i:"💬",c:"#8b5cf6"},
         {l:"Sondaggi",v:cards.filter(function(c){return c.tipo==="sondaggio";}).length,i:"🗳️",c:"#22c55e"},{l:"Quiz",v:cards.filter(function(c){return c.tipo==="quiz";}).length,i:"🧩",c:"#a855f7"},
      ...
      h("div",{style:{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}},
        h("button",{onClick:function(){setShowWordCloud(true);},style:{background:"rgba(99,102,241,.15)",border:"1px solid rgba(99,102,241,.3)",borderRadius:9,padding:"8px 14px",cursor:"pointer",fontSize:12,fontWeight:700,color:"#a5b4fc"}},"☁️ Analisi parole per classe"),
      ...
      (function(){
        var targetLabel = aiTarget==="tutte"?"tutte le classi":aiTarget.slice(7);
        var stats=collectCloudStats(cards,aiTarget);
        return h("div",{style:{display:"flex",gap:12,flexWrap:"wrap",marginBottom:14,padding:"14px",borderRadius:12,background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)"}},
          h("span",{style:{fontSize:12,color:"rgba(255,255,255,.7)"}},"Selezione: "+targetLabel),
          h("span",{style:{fontSize:12,color:"rgba(255,255,255,.7)"}},"Card analizzate: "+stats.cardCount),
          h("span",{style:{fontSize:12,color:"rgba(255,255,255,.7)"}},"Commenti totali: "+stats.commentCount),
          h("span",{style:{fontSize:12,color:"rgba(255,255,255,.7)"}},"Studenti coinvolti: "+stats.studentCount)
        );
      })(),
      h("div",{style:{background:"rgba(255,255,255,.05)",borderRadius:11,padding:16,border:"1px solid rgba(255,255,255,.08)"}},
        h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}},
          h("div",{style:{fontWeight:700,color:"#f1f5f9",fontSize:14}},"🤖 Analisi per classe"),
          h("div",{style:{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}},
            h("select",{value:aiTarget,onChange:function(e){setAIT(e.target.value);setAiR(null);setAiErr("");},style:{padding:"5px 10px",border:"1px solid rgba(255,255,255,.15)",borderRadius:8,fontSize:12,color:"#f1f5f9",background:"rgba(30,41,59,1)",maxWidth:260}},
              h("option",{value:"tutte"},"📋 Tutte le card"),
              aiCardClasses.map(function(cl){return h("option",{key:cl,value:"classe_"+cl},"🏫 Classe "+cl);})
            ),
    ...
    showModal&&h("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",backdropFilter:"blur(6px)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"},onClick:function(){setSM(false);setEditMode(null);}},
      h("div",{style:{background:"#1c1a2e",borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxWidth:540,maxHeight:"90vh",overflow:"auto",border:"1px solid rgba(255,255,255,.1)",borderBottom:"none"},onClick:function(e){e.stopPropagation();}},
        ...
        isProf&&h("div",{style:{display:"flex",gap:5,marginBottom:14,flexWrap:"wrap"}},
          [{v:"domanda",i:"💬"},{v:"sondaggio",i:"🗳️"},{v:"quiz",i:"🧩"}].map(function(t){
