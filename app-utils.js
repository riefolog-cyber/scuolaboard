...
SB.loadAiModule=function(cb){
  ...
  s.onerror=function(){
    var cbs=SB.AI_LOADING||[];
    SB.AI_LOADING=null;
    SB.AI_LOADED=false; // allow retry
...
function fbClassiSave(arr, anno){return db.collection("config").doc("classi_custom_"+safeDocId(anno||"")).set({lista:arr,aggiornato:new Date().toISOString()},{merge:true});}
function fbNascosteSave(arr, anno){return db.collection("config").doc("classi_custom_"+safeDocId(anno||"")).set({nascoste:arr,aggiornato:new Date().toISOString()},{merge:true});}
...
var FORM0={tipo:"domanda",titolo:"",testo:"",opzioni:["",""],links:[{url:"",label:""}],classi:[],quizDomande:[],quizTimer:10,pubblicaIl:"",immagini:[],copertina:null,allegati:[]};

// ── CACHE DURATA: centralizzata ──
var AI_CACHE_TTL=15*60*1000; // 15 minuti

// ── ANNO DEFAULT: centralizzato (da SB_CONFIG) ──
var ANNI_DISPONIBILI = (window.SB_CONFIG && window.SB_CONFIG.ANNI_DISPONIBILI) || ["2025/2026","2026/2027","2027/2028","2028/2029","2029/2030"];
...
function compressImage(file,maxW,maxH,quality){
  if(!file.type.startsWith("image/")) return Promise.reject(new Error("Il file selezionato non è un'immagine valida."));
  return new Promise(function(resolve,reject){
    ...
    img.onload=function(){
      ...
      if(kb>TARGET_KB){b64=canvas.toDataURL("image/jpeg",q*0.65);kb=Math.round(b64.length*0.75/1024);}
      if(kb>TARGET_KB*2){var c2=document.createElement("canvas");c2.width=Math.round(cw*0.75);c2.height=Math.round(ch*0.75);c2.getContext("2d").drawImage(canvas,0,0,c2.width,c2.height);b64=c2.toDataURL("image/jpeg",0.6);kb=Math.round(b64.length*0.75/1024);}
      if(kb>TARGET_KB*3){b64=canvas.toDataURL("image/jpeg",0.4);kb=Math.round(b64.length*0.75/1024);}
      resolve(b64);
    };
...
function renderLinks(card, setShowCard){ // Aggiunto setShowCard come parametro
  ...
  function move(i,delta){
    ...
    if (saveCardToFirebase) {
      ...
      if (setShowCard) { // Aggiorna lo stato locale della card aperta
  ...
  return h("div",{style:{marginTop:8,display:"flex",flexDirection:"column",gap:5}},
    links.map(function(l,i){
      ...
      return h("div",{key:l.url+"-"+i,style:{display:"flex",alignItems:"center",gap:4}},
        h("a",{href:l.url,target:"_blank",rel:"noopener noreferrer",
          style:{flex:1,display:"inline-flex",alignItems:"center",gap:6,background:"rgba(59,130,246,.15)",color:"#60a5fa",padding:"5px 10px",borderRadius:7,textDecoration:"none",fontSize:11,fontWeight:600,border:"1px solid rgba(59,130,246,.3)"}},
          "🔗 "+(l.label||"Approfondisci")
        ),
        links.length>1 && h("button",{onClick:function(e){e.stopPropagation(); move(i,-1);},style:{background:"none",border:"none",cursor:"pointer",color:"rgba(255,255,255,.5)",fontSize:16,padding:0,width:20,height:20,display:"flex",alignItems:"center",justifyContent:"center"}},"⬆️"),
        links.length>1 && h("button",{onClick:function(e){e.stopPropagation(); move(i,1);},style:{background:"none",border:"none",cursor:"pointer",color:"rgba(255,255,255,.5)",fontSize:16,padding:0,width:20,height:20,display:"flex",alignItems:"center",justifyContent:"center"}},"⬇️")
      );
    })
  );
...
(function(){
  ...
  SB.LS={
    ...
    push:{
      get:function(){return _safeGetItem(localStorage, CFG_LS_KEYS.push || "push_enabled")==="true";},
      set:function(v){_safeSetItem(localStorage, CFG_LS_KEYS.push || "push_enabled", v?"true":"false");},
      rm:function(){
        try{
          _safeRemoveItem(localStorage, CFG_LS_KEYS.push || "push_enabled");
        }catch(e){}
      }
    },
    privacy:{
      get:function(uid){return _safeGetItem(localStorage, (CFG_LS_KEYS.privacy && CFG_LS_KEYS.privacy(uid)) || ("privacy_accepted_"+uid));},
      set:function(uid){_safeSetItem(localStorage, (CFG_LS_KEYS.privacy && CFG_LS_KEYS.privacy(uid)) || ("privacy_accepted_"+uid), "1");},
      rm:function(uid){
        try{
    ...
    anno:{
      get:function(){
        try{
          return _safeGetItem(sessionStorage, CFG_LS_KEYS.anno || "annoScolasticoAttivo")||(window.SB_CONFIG && window.SB_CONFIG.ANNO_DEFAULT)||"2026/2027";
        }catch(e){
    ...
    aiCache:{
      get:function(){return _safeGetItem(sessionStorage, CFG_LS_KEYS.aiCache || "ai_results_cache");},
      set:function(v){try{_safeSetItem(sessionStorage, CFG_LS_KEYS.aiCache || "ai_results_cache",v);}catch(e){}},
      rm:function(){try{_safeRemoveItem(sessionStorage, CFG_LS_KEYS.aiCache || "ai_results_cache");}catch(e){}}
    },
    aiCacheAt:{
      get:function(){return _safeGetItem(sessionStorage, CFG_LS_KEYS.aiCacheAt || "ai_results_cache_at");},
      set:function(v){try{_safeSetItem(sessionStorage, CFG_LS_KEYS.aiCacheAt || "ai_results_cache_at",v);}catch(e){}},
      rm:function(){try{_safeRemoveItem(sessionStorage, CFG_LS_KEYS.aiCacheAt || "ai_results_cache_at");}catch(e){}}
    }
  };
...
window.cleanMarkdownText = SB.cleanMarkdownText;
