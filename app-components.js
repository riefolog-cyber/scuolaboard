      ...
      h("div", { style: { background: cmBg, borderRadius: 9, padding: "8px 10px", border: "1px solid " + cmBorder } },
        h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 } },
          ...
          h("div", { style: { display: "flex", gap: 6, alignItems: "center" } },
            ...
            (isMyCm || isProf) && !isEditing && h("button", { onClick: function () { setEditingCm({ id: item.id, testo: item.testo }); }, className: "btn btn-ghost", style: { color: "rgba(99,102,241,.7)", padding: "2px 6px" } }, "✏️"),
            isProf && !simulaSt && !isMyCm && h("button", { onClick: function () { setShowAmm({ cardId: showCard.id, cmId: item.id, autore: item.autore }); }, className: "btn btn-ghost", style: { color: "rgba(245,158,11,.6)", padding: "2px 6px" } }, "⚠️"),
            (isMyCm || isProf) && h("button", { onClick: function () { setConfirmDel(isTopLevel ? { type: "comment", cardId: showCard.id, id: item.id } : { type: "reply", cmId: parentId || item.id, id: item.id, cardId: showCard.id }); }, className: "btn btn-ghost", style: { color: "rgba(255,255,255,.4)", padding: "2px 6px", fontSize: 15 } }, "×")
          )
        ),
        isEditing ? h("div", null,
          ...
          h("div", { style: { display: "flex", gap: 6 } },
            h("button", { onClick: function () { saveEditCm(showCard.id); }, className: "btn btn-primary", style: { flex: 1 } }, "Salva"),
            h("button", { onClick: function () { setEditingCm(null); }, className: "btn btn-ghost", style: { padding: "5px 10px" } }, "Annulla")
          )
        )
        : h("div", null,
            h("div", { style: { fontSize: depth === 0 ? 12 : 11, color: "rgba(255,255,255,.65)", lineHeight: 1.5, marginBottom: 5 } },
              item.testo,
              (item.allegati || []).map(function(allegato){
                var isImage = allegato.type && allegato.type.startsWith("image/");
                var isPdf = allegato.type === "application/pdf";
                var isHtml = allegato.type === "text/html";
                var icon = "📄";
                if (isImage) icon = "🖼️";
                else if (isPdf) icon = "📄";
                else if (isHtml) icon = "🌐";
                ...
                return h("div", { key: allegato.id, style: { marginTop: 10, background: "rgba(255,255,255,.05)", borderRadius: 7, padding: 8, border: "1px solid rgba(255,255,255,.08)" } },
                  h("a", { href: safeUrl, target: "_blank", rel: "noopener noreferrer", style: { display: "flex", alignItems: "center", gap: 8, color: "#a5b4fc", textDecoration: "none", fontWeight: 600 } },
                    h("span", null, icon),
                    h("span", null, allegato.name)
                  ),
                  isImage && h("img", { src: safeUrl, alt: allegato.name, style: { maxWidth: "100%", maxHeight: 200, borderRadius: 5, marginTop: 8 } }),
            ...
            h("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", alignItems: "center", marginBottom: 4 } },
              ...
              h("button", { onClick: function () { setReplyTo({ id: item.id, autore: item.autore }); setReplyTesto(""); }, className: "btn btn-ghost btn-sm", style: { color: "#a5b4fc" } }, "↩️ Rispondi")
            )
        ),
        isReplying && h("div", { style: { marginLeft: 14, marginTop: 5, background: "rgba(99,102,241,.07)", borderRadius: 8, padding: 8, border: "1px solid rgba(99,102,241,.18)" } },
          ...
          h("div", { style: { display: "flex", gap: 6 } },
            h("button", { onClick: function () { addReply(item.id); }, className: "btn btn-primary", style: { flex: 1, opacity: replyTesto.trim() ? 1 : 0.4, cursor: replyTesto.trim() ? "pointer" : "not-allowed" } }, "Invia"),
            h("button", { onClick: function () { setReplyTo(null); setReplyTesto(""); }, className: "btn btn-ghost", style: { padding: "5px 10px" } }, "Annulla")
          )
        ),
