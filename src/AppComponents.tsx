// app-components.js  ·  ScuolaBoard  ·  Sotto-componenti ottimizzati

var SB = window.SB || {};
window.SB = SB;

var h = SB.h || React.createElement;
var Fragment = SB.Fragment || React.Fragment;
var useState = SB.useState || React.useState;
var _useCountUp = SB.useCountUp; // alias per evitare TS2300 con useCountUp globale da app-utils.ts

// 1. STAT CARD (Estratto dal render principale per evitare re-mount e reset degli hook)
function StatCard(props) {
  var n = _useCountUp(props.s.v, 600);
  return (
    <div
      key={props.s.l}
      style={{
        flex: 1,
        minWidth: 90,
        background: 'rgba(255,255,255,.05)',
        borderRadius: 11,
        padding: '10px 12px',
        border: '1px solid rgba(255,255,255,.08)',
        borderTop: '3px solid ' + props.s.c,
      }}
    >
      {<div style={{ fontSize: 18 }}>{props.s.i}</div>}
      {<div style={{ fontSize: 22, fontWeight: 800, color: '#f1f5f9' }}>{n}</div>}
      {<div style={{ fontSize: 11, color: 'rgba(255,255,255,.58)' }}>{props.s.l}</div>}
    </div>
  );
}

// 2. COMMENT THREAD (Estratto dal render per migliorare le performance di ricalcolo del DOM dei commenti)
function CommentThread(props) {
  var item = props.item;
  var depth = props.depth;
  var parentId = props.parentId;
  var user = props.user;
  var isProf = props.isProf;
  var simulaSt = props.simulaSt;
  var editingCm = props.editingCm;
  var setEditingCm = props.setEditingCm;
  var replyTo = props.replyTo;
  var setReplyTo = props.setReplyTo;
  var replyTesto = props.replyTesto;
  var setReplyTesto = props.setReplyTesto;
  var showCard = props.showCard;
  var setShowAmm = props.setShowAmm;
  var setConfirmDel = props.setConfirmDel;
  var saveEditCm = props.saveEditCm;
  var toggleReaction = props.toggleReaction;
  var addReply = props.addReply;
  var myName = props.myName;

  var vn = myName(user);
  var isMyCm = item.autore && vn && vn !== '?' && item.autore.trim() === vn.trim();
  var isEditing = editingCm && editingCm.id === item.id;
  var isReplying = replyTo && replyTo.id === item.id;
  var isTopLevel = depth === 0;
  var isProf_cm = item.autore === 'Prof' || (item.autore && item.autore.startsWith('Prof'));
  var cmBg = isProf_cm ? 'rgba(99,102,241,.18)' : 'rgba(255,255,255,' + Math.max(0.01, 0.05 - depth * 0.01) + ')';
  var cmBorder = isProf_cm ? 'rgba(99,102,241,.4)' : 'rgba(255,255,255,' + Math.max(0.03, 0.08 - depth * 0.015) + ')';
  var REACTIONS = ['👍', '❤️', '🤔'];

  return (
    <div style={{ marginLeft: Math.min(depth, 4) * 14, marginTop: depth > 0 ? 5 : 10 }}>
      {
        <div style={{ background: cmBg, borderRadius: 9, padding: '8px 10px', border: '1px solid ' + cmBorder }}>
          {
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
              {
                <span
                  style={{
                    fontWeight: 700,
                    fontSize: depth === 0 ? 12 : 11,
                    color: isProf_cm ? '#818cf8' : isMyCm ? '#a5b4fc' : 'rgba(255,255,255,.55)',
                    display: 'flex',
                    alignItems: 'center',
                    gap: 5,
                  }}
                >
                  {depth > 0 && <span style={{ color: 'rgba(99,102,241,.5)', marginRight: 4, fontSize: 11 }}>↳</span>}
                  {depth === 0 && Avatar(item.autore, 22)}
                  {item.autore + (item.modificato ? ' ✎' : '')}
                  {isProf_cm && (
                    <span
                      style={{
                        background: 'rgba(99,102,241,.3)',
                        color: '#c7d2fe',
                        borderRadius: 4,
                        padding: '0px 5px',
                        fontSize: 11,
                        fontWeight: 800,
                      }}
                    >
                      PROF
                    </span>
                  )}
                </span>
              }
              {
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  {
                    <span title={fmtDT(item.data)} style={{ fontSize: 11, color: 'rgba(255,255,255,.2)' }}>
                      {timeAgo(item.data)}
                    </span>
                  }
                  {(isMyCm || isProf) && !isEditing && (
                    <button
                      onClick={function () {
                        setEditingCm({ id: item.id, testo: item.testo });
                      }}
                      className="btn btn-ghost"
                      style={{ color: 'rgba(99,102,241,.7)', padding: '2px 6px' }}
                    >
                      ✏️
                    </button>
                  )}
                  {isProf && !simulaSt && !isMyCm && (
                    <button
                      onClick={function () {
                        setShowAmm({ cardId: showCard.id, cmId: item.id, autore: item.autore });
                      }}
                      className="btn btn-ghost"
                      style={{ color: 'rgba(245,158,11,.6)', padding: '2px 6px' }}
                    >
                      ⚠️
                    </button>
                  )}
                  {(isMyCm || isProf) && (
                    <button
                      onClick={function () {
                        setConfirmDel(
                          isTopLevel
                            ? { type: 'comment', cardId: showCard.id, id: item.id }
                            : { type: 'reply', cmId: parentId || item.id, id: item.id, cardId: showCard.id }
                        );
                      }}
                      className="btn btn-ghost"
                      style={{ color: 'rgba(255,255,255,.4)', padding: '2px 6px', fontSize: 15 }}
                    >
                      ×
                    </button>
                  )}
                </div>
              }
            </div>
          }
          {isEditing ? (
            <div>
              {
                <textarea
                  value={editingCm.testo}
                  onInput={function (e) {
                    setEditingCm(function (p) {
                      return Object.assign({}, p, { testo: e.target.value });
                    });
                  }}
                  rows={2}
                  style={Object.assign({}, S.input, { resize: 'none', marginBottom: 6, fontSize: 12 })}
                />
              }
              {
                <div style={{ display: 'flex', gap: 6 }}>
                  {
                    <button
                      onClick={function () {
                        saveEditCm(showCard.id);
                      }}
                      className="btn btn-primary"
                      style={{ flex: 1 }}
                    >
                      Salva
                    </button>
                  }
                  {
                    <button
                      onClick={function () {
                        setEditingCm(null);
                      }}
                      className="btn btn-ghost"
                      style={{ padding: '5px 10px' }}
                    >
                      Annulla
                    </button>
                  }
                </div>
              }
            </div>
          ) : (
            <div>
              {
                <div
                  style={{
                    fontSize: depth === 0 ? 12 : 11,
                    color: 'rgba(255,255,255,.65)',
                    lineHeight: 1.5,
                    marginBottom: 5,
                  }}
                >
                  {item.testo}
                  {(item.allegati || []).map(function (allegato) {
                    var isImage = allegato.type && allegato.type.startsWith('image/');
                    var isPdf = allegato.type === 'application/pdf';
                    var isHtml = allegato.type === 'text/html';
                    var icon = '📄';
                    if (isImage) icon = '🖼️';
                    else if (isPdf) icon = '📄';
                    else if (isHtml) icon = '🌐';

                    // Validazione URL per href/src: solo https:, data:image/*, data:application/pdf
                    var safeUrl = '#';
                    try {
                      var u = allegato.url || '';
                      if (
                        u.startsWith('https://') ||
                        u.startsWith('data:image/') ||
                        u.startsWith('data:application/pdf')
                      ) {
                        safeUrl = u;
                      }
                    } catch (e) {
                      safeUrl = '#';
                    }

                    return (
                      <div
                        key={allegato.id}
                        style={{
                          marginTop: 10,
                          background: 'rgba(255,255,255,.05)',
                          borderRadius: 7,
                          padding: 8,
                          border: '1px solid rgba(255,255,255,.08)',
                        }}
                      >
                        {
                          <a
                            href={safeUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            style={{
                              display: 'flex',
                              alignItems: 'center',
                              gap: 8,
                              color: '#a5b4fc',
                              textDecoration: 'none',
                              fontWeight: 600,
                            }}
                          >
                            {<span>{icon}</span>}
                            {<span>{allegato.name}</span>}
                          </a>
                        }
                        {isImage && (
                          <img
                            src={safeUrl}
                            alt={allegato.name}
                            style={{ maxWidth: '100%', maxHeight: 200, borderRadius: 5, marginTop: 8 }}
                          />
                        )}
                        {isHtml && (
                          <div
                            style={{
                              marginTop: 8,
                              padding: '6px 8px',
                              background: 'rgba(245,158,11,.1)',
                              border: '1px solid rgba(245,158,11,.25)',
                              borderRadius: 5,
                              fontSize: 11,
                              color: '#fbbf24',
                            }}
                          >
                            ⚠️ I file HTML non vengono visualizzati inline per sicurezza. Scarica il file per aprirlo.
                          </div>
                        )}
                      </div>
                    );
                  })}
                </div>
              }
              {
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', alignItems: 'center', marginBottom: 4 }}>
                  {REACTIONS.map(function (emoji) {
                    var chi = (item.reazioni && item.reazioni[emoji]) || [];
                    var mine = chi.indexOf(myName(user)) >= 0;
                    return (
                      <button
                        key={emoji}
                        onClick={function () {
                          toggleReaction(showCard.id, item.id, emoji);
                        }}
                        style={{
                          background: mine ? 'rgba(99,102,241,.25)' : 'rgba(255,255,255,.05)',
                          border: '1px solid ' + (mine ? 'rgba(99,102,241,.4)' : 'rgba(255,255,255,.08)'),
                          borderRadius: 20,
                          padding: '1px 7px',
                          cursor: 'pointer',
                          fontSize: 11,
                          color: mine ? '#a5b4fc' : 'rgba(255,255,255,.45)',
                          fontWeight: mine ? 700 : 400,
                          display: 'flex',
                          alignItems: 'center',
                          gap: 3,
                        }}
                      >
                        {emoji}
                        {chi.length > 0 && <span style={{ fontSize: 11, fontWeight: 700 }}>{chi.length}</span>}
                      </button>
                    );
                  })}
                  {
                    <button
                      onClick={function () {
                        setReplyTo({ id: item.id, autore: item.autore });
                        setReplyTesto('');
                      }}
                      className="btn btn-ghost btn-sm"
                      style={{ color: '#a5b4fc' }}
                    >
                      ↩️ Rispondi
                    </button>
                  }
                </div>
              }
            </div>
          )}
          {isReplying && (
            <div
              style={{
                marginLeft: 14,
                marginTop: 5,
                background: 'rgba(99,102,241,.07)',
                borderRadius: 8,
                padding: 8,
                border: '1px solid rgba(99,102,241,.18)',
              }}
            >
              {
                <div style={{ fontSize: 11, color: '#a5b4fc', marginBottom: 4, fontWeight: 700 }}>
                  {'↩️ Rispondi a ' + replyTo.autore}
                </div>
              }
              {
                <textarea
                  value={replyTesto}
                  onInput={function (e) {
                    setReplyTesto(e.target.value);
                  }}
                  rows={2}
                  placeholder="Scrivi una risposta…"
                  style={Object.assign({}, S.input, { resize: 'none', marginBottom: 6, fontSize: 11 })}
                />
              }
              {
                <div style={{ display: 'flex', gap: 6 }}>
                  {
                    <button
                      onClick={function () {
                        addReply(item.id);
                      }}
                      className="btn btn-primary"
                      style={{
                        flex: 1,
                        opacity: replyTesto.trim() ? 1 : 0.4,
                        cursor: replyTesto.trim() ? 'pointer' : 'not-allowed',
                      }}
                    >
                      Invia
                    </button>
                  }
                  {
                    <button
                      onClick={function () {
                        setReplyTo(null);
                        setReplyTesto('');
                      }}
                      className="btn btn-ghost"
                      style={{ padding: '5px 10px' }}
                    >
                      Annulla
                    </button>
                  }
                </div>
              }
            </div>
          )}
          {(item.risposte || []).length > 0 && (
            <div>
              {(item.risposte || []).map(function (r) {
                return (
                  <CommentThread
                    key={r.id}
                    item={r}
                    depth={depth + 1}
                    parentId={item.id}
                    user={user}
                    isProf={isProf}
                    simulaSt={simulaSt}
                    editingCm={editingCm}
                    setEditingCm={setEditingCm}
                    replyTo={replyTo}
                    setReplyTo={setReplyTo}
                    replyTesto={replyTesto}
                    setReplyTesto={setReplyTesto}
                    showCard={showCard}
                    setShowAmm={setShowAmm}
                    setConfirmDel={setConfirmDel}
                    saveEditCm={saveEditCm}
                    toggleReaction={toggleReaction}
                    addReply={addReply}
                    myName={myName}
                  />
                );
              })}
            </div>
          )}
        </div>
      }
    </div>
  );
}

SB.StatCard = SB.memo || React.memo ? (SB.memo || React.memo)(StatCard) : StatCard;
SB.CommentThread = SB.memo || React.memo ? (SB.memo || React.memo)(CommentThread) : CommentThread;
