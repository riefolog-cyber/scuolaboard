...
(function(){
  ...
  function ClasseModal(props) {
    ...
    var classeCorrente = props.classeCorrente; // Nuova prop

    var isDisabled = !!classeCorrente; // Se esiste una classe corrente, disabilita
    ...
    return h("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20},onClick:function(){props.setShowClasseModal(false);}},
      h("div",{style:{background:"#1c1a2e",border:"1px solid rgba(99,102,241,.3)",borderRadius:20,padding:28,maxWidth:440,width:"100%"},onClick:function(e){e.stopPropagation();}},
        h("div",{style:{textAlign:"center",marginBottom:16}},
          ...
          h("div",{style:{fontSize:12,color:"rgba(255,255,255,.45)",lineHeight:1.6}},
            isDisabled
        ...
        h("div",{style:{marginBottom:16}},
          ...
          isDisabled
            ?h("div",{style:Object.assign({},S.input,{fontSize:14,color:"#f1f5f9",background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)"})},classeCorrente)
        ...
        h("button",{onClick:function(){props.saveClasse();},disabled:isDisabled||!props.classeInput,style:{width:"100%",padding:13,background:(isDisabled||!props.classeInput)?"rgba(255,255,255,.06)":"linear-gradient(135deg,#6366f1,#8b5cf6)",color:(isDisabled||!props.classeInput)?"rgba(255,255,255,.40)":"#fff",border:"none",borderRadius:12,fontSize:14,fontWeight:800,cursor:(isDisabled||!props.classeInput)?"not-allowed":"pointer"}},
          ...
          h("div",{style:{fontSize:11,color:"rgba(255,255,255,.58)",marginBottom:6,fontWeight:700}},"MOTIVAZIONE"),
            h("div",{style:{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8}},
              ["Commento non pertinente","Linguaggio inappropriato","Risposta superficiale o vuota","Mancanza di rispetto","Copia da un compagno","Non partecipa alla discussione"].map(function(m){
                return h("button",{key:m,onClick:function(){var el=document.getElementById("amm-input");if(el)el.value=m;},style:{background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",borderRadius:20,padding:"3px 10px",cursor:"pointer",fontSize:11,color:"rgba(255,255,255,.65)"}},m);
              })
            ),
  ...
  function NuovaCardModal(props) {
    ...
    var handleAllegatiUpload=props.handleAllegatiUpload,handleRimuoviAllegato=props.handleRimuoviAllegato;
    var allegatiUploading=props.allegatiUploading,setAllegatiUploading=props.setAllegatiUploading,showToast=props.showToast;
    return h("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",backdropFilter:"blur(6px)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"},onClick:function(){setShowModal(false);if(editMode)setEditMode(null);}},
      h("div",{style:{background:"#1c1a2e",borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxWidth:540,maxHeight:"90vh",overflow:"auto",border:"1px solid rgba(255,255,255,.1)",borderBottom:"none"},onClick:function(e){e.stopPropagation();}},
        ...
        h("div",{style:{marginBottom:10}},h("label",{style:{fontSize:11,fontWeight:700,color:"rgba(255,255,255,.58)",display:"block",marginBottom:4,letterSpacing:1}},"TITOLO *"),h("input",{value:form.titolo,maxLength:100,onInput:function(e){setForm(function(p){return Object.assign({},p,{titolo:e.target.value});});},placeholder:"Es. Riflessione su…",style:S.input})),
        ...
        h("div",{style:{marginBottom:10}},
          h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}},
            h("label",{style:{fontSize:11,fontWeight:700,color:"rgba(255,255,255,.58)",letterSpacing:1}},"📎 ALLEGATI"),
            (function(){var kb=0;(form.allegati||[]).forEach(function(a){kb+=Math.round((a.size||0)/1024);});var color=kb>500?"#f87171":kb>200?"#fbbf24":"rgba(255,255,255,.40)";var warn=kb>500?" ⚠️ limite 500KB":kb>200?" — attento alle dimensioni":"";return kb>0?h("span",{style:{fontSize:11,color:color,fontWeight:700}},kb+" KB"+warn):h("span",{style:{fontSize:11,color:"rgba(255,255,255,.40)"}},"Max 500KB totali");})()
          ),
          h("label",{style:{display:"flex",alignItems:"center",gap:6,background:"rgba(255,255,255,.04)",border:"1px dashed rgba(255,255,255,.1)",borderRadius:8,padding:"6px 10px",cursor:"pointer",fontSize:11,color:"rgba(255,255,255,.52)"}},
            "📄",allegatiUploading?"Caricamento…":"+ Aggiungi file",
            h("input",{type:"file",accept:".pdf,.doc,.docx,.txt,.md,.csv,.xls,.xlsx,.ppt,.pptx,.zip,.rar",multiple:true,style:{display:"none"},disabled:allegatiUploading,onChange:function(e){handleAllegatiUpload(e);}})
          ),
          (form.allegati||[]).length>0&&h("div",{style:{display:"flex",flexDirection:"column",gap:4,marginTop:8}},(form.allegati||[]).map(function(a){
            var icon=a.type&&a.type.startsWith("image")?"🖼️":a.type&&a.type.startsWith("application/pdf")?"📄":a.type&&a.type.includes("word")?"📝":a.type&&a.type.includes("spreadsheet")?"📊":a.type&&a.type.includes("presentation")?"📽️":"📎";
            return h("div",{key:a.id,style:{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.08)",borderRadius:8,padding:"6px 10px"}},
              h("span",{style:{fontSize:16}},icon),
              h("div",{style:{flex:1,minWidth:0}},h("div",{style:{fontSize:12,color:"#f1f5f9",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}},a.name),h("div",{style:{fontSize:10,color:"rgba(255,255,255,.45)"}},Math.round(a.size/1024)+" KB")),
              h("button",{onClick:function(){handleRimuoviAllegato(a.id);},style:{background:"rgba(239,68,68,.2)",color:"#f87171",border:"none",borderRadius:6,width:24,height:24,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}},"×")
            );
          }))
        ),
  ...
  function ConfirmDelModal(props) {
    ...
    var modalMessage = "Vuoi davvero eliminare " + (isComment ? "questo commento" : isReply ? "questa risposta" : "questa card") + "? L'azione può essere annullata entro 5 secondi.";
    ...
    return h("div", { style: { position: "fixed", inset: 0, background: "rgba(0,0,0,.82)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }, onClick: function() { setConfirmDel(null); } },
      h("div", { style: { background: "#1c1a2e", border: "1px solid rgba(239,68,68,.35)", borderRadius: 20, padding: 26, maxWidth: 360, width: "100%" }, onClick: function(e) { e.stopPropagation(); } },
        h("h3", { style: { margin: "0 0 4px", color: "#f87171", fontSize: 15, fontWeight: 800, textAlign: "center" } }, modalTitle),
        h("p", { style: { color: "rgba(255,255,255,.45)", fontSize: 12, marginBottom: 14, textAlign: "center", lineHeight: 1.5 } }, modalMessage),
        h("div", { style: { display: "flex", gap: 10 } },
          h("button", { onClick: function() { setConfirmDel(null); }, style: { flex: 1, padding: 11, background: "rgba(255,255,255,.08)", color: "rgba(255,255,255,.6)", border: "none", borderRadius: 11, fontSize: 13, fontWeight: 700, cursor: "pointer" } }, "Annulla"),
          h("button", { onClick: confirmAction, style: { flex: 2, padding: 11, background: "linear-gradient(135deg,#ef4444,#f87171)", color: "#fff", border: "none", borderRadius: 11, fontSize: 14, fontWeight: 800, cursor: "pointer" } }, "🗑️ Elimina")
        )
      )
    );
  }
